...
| 阶段 | 目标 | 主要动作 | 完成条件 |
| --- | --- | --- | --- |
| Stage 0 | 接口事实清单 | 总控先选择一个 `Interface Area`，扫描该区域 known breaks、typecheck / build / lint 失败、重复 contract 和 runtime sync 点。 | 形成该 area 的 `Area Plan` 与 producer / consumer / evidence 清单，不改代码。 |
| Stage 1 | contract anchor 选择 | 为该 area 的接口面选 API report、schema snapshot、runtime export list 或 fixture schema。 | anchor 可生成、可 diff、可被总控复核。 |
| Stage 2 | 第一批最小接口修复 | 从该 area 内只选择失败证据明确、影响面小、验证命令可运行的一个 dossier。 | 每个修复有最小 diff 和 producer + consumer 验证。 |
| Stage 3 | 区域内同步收敛 | 对该 area 内 source contract 与 runtime package / generated client / fixture 不一致的项做同步。 | 生成命令和 diff 可复核，不跨 area 扩大。 |
...
| Stage 5 | 自动化候选规则固化 | 把已验证可无人值守的 area + dossier 类型沉淀为总控 TODO 领取边界。 | 不新增流程，只补现有总控任务模板 / checklist。 |
...
- 第一批只允许从 build / typecheck / lint / targeted test 明确失败，或 source / runtime / fixture 明确 drift 中选择。
